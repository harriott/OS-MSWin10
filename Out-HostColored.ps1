...
# :%s/ \+$  and I concatenated some comment lines

# GENERIC INSTALLATION HELPER CODE - deleted

# sourced in  $MSWin10\PSProfile.ps1

# Revisions  https://gist.github.com/mklement0/243ea8297e7db0e1c03a67ce4b1e765d/revisions
# > @mklement0 mklement0 revised this gist on 24 Apr 2021.

# try some  .EXAMPLE,  below
# ---------
...
<#
  ...
  The above directly defines the function below in your session and offers guidance for making it available in future sessions too.
...
.DESCRIPTION
Colors portions of the default-formatted host output based on either regular expressions or a literal substrings, assuming the host is a console or supports colored output using console colors.
...
Two basic syntax forms are supported:
  ...
  * Multi-color (color per pattern), via a hashtable (dictionary) passed to -PatternColorMap.

Note: Since output is sent to the host rather than the pipeline, you cannot chain calls to this function.

.PARAMETER Pattern
One or more search patterns specifying what parts of the formatted representations of the input objects should be colored.

 * By default, these patterns are interpreted as regular expressions.

 * If -SimpleMatch is also specified, the patterns are interpreted as literal substrings.

.PARAMETER ForegroundColor
The foreground color to use for the matching portions.  Defaults to yellow.
...
<pattern-or-pattern-array> is either a single string or an array of strings specifying the regex pattern(s) or literal substring(s) (with -SimpleMatch) to match.
Note: If you're specifying an array in a hashtable literal, you must enclose it in (...), and the individual patterns must all be quoted; e.g.: ('foo', 'bar')

<color-spec> is a string that contains either a foreground [ConsoleColor] color alone (e.g. 'red'), a combination with a background color separated by "," (e.g., 'red,white') or just a background color (e.g, ',white').

See the examples for a complete example.

.PARAMETER CaseSensitive
Matches the patterns case-sensitively.  By default, matching is case-insensitive.

.PARAMETER WholeLine
Specifies that the entire line containing a match should be colored, not just the matching portion.

.PARAMETER SimpleMatch
Interprets the -Pattern argument(s) as a literal substrings to match rather than as regular expressions.

.PARAMETER InputObject
The input object(s) whose formatted representations to color selectively.  Typically provided via the pipeline.
...
Outputs the current date with all tokens composed of letters (p{L}) only in red on a white background.

.EXAMPLE
Get-Date | Out-HostColored @{ '\p{L}+' = 'red,white' }

Same as the previous example, only via the dictionary-based -PatternColorMap parameter (implied).
...
Note the need to enclose pattern array 'easy', 'green' in (...), which also necessitates quoting its element.
...
#>
  ...
  begin {
    ...
    if ($PSCmdlet.ParameterSetName -eq 'SingleColor') {
      ...
      # Translate the indiv. arguments into the dictionary format suppoorted by -PatternColorMap, so we can process $PatternColorMap uniformly below.
      $PatternColorMap = @{
    ...
    try {
      ...
      #  * Keys: Consolidate multiple patterns into a single one with alternation and #          construct a [regex] instance from it.
      #  * Values: Transform the "[foregroundColor],[backgroundColor]" strings into an arguments hashtable that can be used for splatting with Write-Host.
      $map = @{ }
    ...
    # Construct the script block that is used in the steppable pipeline created further below.
    $scriptCmd = {

      # Format the input objects with Out-String and output the results line by line, then look for matches and color them.
    ...
    # Create the script block as a *steppable pipeline*, which enables to perform regular streaming pipeline processing, without having to collect everything in memory first.
